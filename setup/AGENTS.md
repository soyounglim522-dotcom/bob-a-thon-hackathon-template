...
Test cases in `tests/*.json` — see [`wxo-agent/skills/wxo-adk-agent/references/evaluation_template.json`](../wxo-agent/skills/wxo-adk-agent/references/evaluation_template.json). Each `goal_details` entry is either `type: tool_call` (arg matching: `strict`/`fuzzy`/`optional`) or `type: text` (keyword matching). All goals must pass.
